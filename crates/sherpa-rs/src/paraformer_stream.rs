...
impl ParaFormerStream {
    pub fn new(config: OnlineParaformerConfig) -> Result<Self> {
        // ParaFormerStream config
...
unsafe impl Send for ParaFormerStream {}
unsafe impl Sync for ParaFormerStream {}

impl Drop for ParaFormerStream {
    fn drop(&mut self) {
        unsafe {
